...
export class Base {
  ...
  async sendMessage(
    ...
    try {
      ...
      // -- SYNC OUTGOING MESSAGE TO CRM DASHBOARD --
      try {
        let sentText = "";
        let sentType = Object.keys(message)[0] || "text";

        if (message.text) {
          sentText = message.text.body;
        } else if (message.interactive) {
          sentText = message.interactive.body?.text || message.interactive.header?.text || "[Interactive Button/List Message]";
          if (message.interactive.header?.type === 'image') sentType = 'image';
          else if (message.interactive.header?.type === 'video') sentType = 'video';
          else sentType = 'interactive';
        } else if (message.template) {
          sentType = "template";
          sentText = `[Template: ${message.template.name}]`;
        } else if (message.image) {
          sentText = message.image.caption || "[Image]";
        } else if (message.audio) {
          sentText = "[Audio Message]";
        } else if (message.video) {
          sentText = message.video.caption || "[Video]";
        } else if (message.document) {
          sentText = "[Document]";
        } else {
          sentText = `[${sentType} message]`;
        }

        const messageId = response.data?.messages?.[0]?.id || `bot_msg_${Date.now()}`;

        axios.post('http://localhost:3001/api/bot-message', {
          whatsapp_message_id: messageId,
          to_number: id,
          message_text: sentText,
          message_type: sentType
        }, { timeout: 3000 }).catch(e => console.error("⚠️ Failed to sync bot message to dashboard:", e.message));

      } catch (syncError) {
        console.error("Error syncing to dashboard:", syncError);
      }
      // --------------------------------------------
  ...
  async sendAudioFromUrl(
    ...
    try {
      ...
      if (!isUrl(url)) {
        throw new Error("Invalid URL provided for audio.");
      }

      // Upload the audio and get the media ID
      const { id: mediaId } = await this.uploadMedia(url);
  ...
  async sendBtn(
    data: {
      text: string;
      buttons: { url: string; title: string; type: string }[];
    },
    contextOptions: Record<string, any> = {}
  ): Promise<void> {
    const recipientId = contextOptions.id || this.userId;
  ...
  //Template for url button message
  ...
  async sendTemplate(
    ...
    // If template has variables, add components
    ...
    await this.sendMessage(recipientId, payload);
  }
